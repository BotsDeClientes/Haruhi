...
const handler = async (m, { conn, command, args, text, usedPrefix }) => {
  ...
  const tradutor = _translate.plugins.descargas_🤖;

  if (!text) throw `s_${usedPrefix + command} S`;

  const yt_🤖 = await search(args.join(' '));
  let additionalText = '';
  if (command === '🤖') {
  ...
  conn.sendMessage(m.chat, { image: { url: yt_🤖[0].thumbnail }, caption: texto1 }, { quoted: m });

  if (command === '🤖') {
    try {
      const { status, resultados, error } = await ytmp33(yt_🤖[0].url);
      ...
      if (size >= limit_a2) {
        await conn.sendMessage(m.chat, { text: `s _${resultados.descargar}_` }, { quoted: m });
        return;
      }
      ...
      try {
        const audio = `${global.MyApiRestBaseUrl}/api/v1/ytmp3?url=${yt_🤖[0].url}&apikey=${global.MyApiRestApikey}`;
        const ttl = await yt_🤖[0].title;
        ...
        if (size >= limit_a2) {
          await conn.sendMessage(m.chat, { text: `s _${audio}_` }, { quoted: m });
          return;
        }
        if (size >= limit_a1 && size <= limit_a2) {
          await conn.sendMessage(m.chat, { document: buff_aud, mimetype: 'audio/mpeg', fileName: ttl + `.mp3` }, { quoted: m });
  ...
  if (command === '🤖2') {
    try {
      const { status, resultados, error } = await ytmp44(yt_🤖[0].url);
      if (!status) throw new Error(error);
      ...
      console.log('Fallo el 1: ' + error);
      try {
        const video = `${global.MyApiRestBaseUrl}/api/v1/ytmp4?url=${yt_🤖[0].url}&apikey=${global.MyApiRestApikey}`;
        const ttl2 = await yt_🤖[0].title;
        ...
        if (size2 >= limit2) {
          await conn.sendMessage(m.chat, { text: `s _${video}_` }, { quoted: m });
          return;
        }
        if (size2 >= limit1 && size2 <= limit2) {
          await conn.sendMessage(m.chat, { document: buff_vid, mimetype: 'video/mp4', fileName: ttl2 + `.mp4` }, { quoted: m });
...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  ...
  if (!text) throw `${tradutor.texto1[0]} _${usedPrefix + command} ${tradutor.texto1[1]}`;
    ...
    const texto1 = `${tradutor.texto2[0]} ${yt_🤖[0].title}\n${tradutor.texto2[1]} ${yt_🤖[0].ago}\n${tradutor.texto2[2]} ${secondString(yt_🤖[0].duration.seconds)}\n${tradutor.texto2[3]} ${`${MilesNumber(yt_🤖[0].views)}`}\n${tradutor.texto2[4]} ${yt_🤖[0].author.name}\n${tradutor.texto2[5]} ${yt_🤖[0].videoId}\n${tradutor.texto2[6]} ${yt_🤖[0].type}\n${tradutor.texto2[7]} ${yt_🤖[0].url}\n${tradutor.texto2[8]} ${yt_🤖[0].author.url}\n\n> ${tradutor.texto2[9]} ${additionalText}. ${tradutor.texto2[10]}`.trim();
    conn.sendMessage(m.chat, {image: {url: yt_🤖[0].thumbnail}, caption: texto1}, {quoted: m});
    if (command == '🤖') {
    try {   
    const audio = `${global.MyApiRestBaseUrl}/api/v1/ytmp3?url=${yt_🤖[0].url}&apikey=${global.MyApiRestApikey}`;
...
function secondString(seconds) {
  ...
  const dDis🤖 = d > 0 ? d + (d == 1 ? 'd ' : 'd ') : '';
  const hDis🤖 = h > 0 ? h + (h == 1 ? 'h ' : 'h ') : '';
  const mDis🤖 = m > 0 ? m + (m == 1 ? 'm ' : 'm ') : '';
  const sDis🤖 = s > 0 ? s + (s == 1 ? 's' : 's') : '';
