...
global.authFile = 'MegatronBotPro'
global.authFileJB  = 'MegatronPro'
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['MegatronPro', 'Edge', '2.0.0'] : methodCodeQR ? ['MegatronPro', 'Edge', '2.0.0'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "MegatronPro"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
const subBotDirectory = './MegatronPro';
...
(async () => {
global.conns = [];
const mainBotAuthFile = 'MegatronBotPro';
try {
...
let prekey = []
let directorio = readdirSync("./MegatronBotPro")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./MegatronBotPro/${files}`)
})
} 
function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./MegatronPro/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./MegatronPro/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./MegatronPro/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./MegatronPro/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./MegatronBotPro/', './MegatronPro/']
